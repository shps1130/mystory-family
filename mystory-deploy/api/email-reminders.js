...
async function sendReminder(firstName, email, dayNumber) {
  ...
  const subject = isDay3
    ? `${firstName}, Grace is waiting for you 🕊️`
    : `Your story deserves to be finished, ${firstName}`;

  const headline = isDay3
    ? `Grace hasn't forgotten you.`
...
<body style="margin:0;padding:0;background:#fdf6ec;font-family:'Georgia',serif;">
  <div style="max-width:560px;margin:0 auto;padding:48px 24px;">
    ...
    <div style="text-align:center;margin-bottom:40px;">
      ...
      <h1 style="font-size:30px;font-weight:300;color:#3d2b1a;font-style:italic;margin:0;">
        ${headline}
      </h1>
    </div>

    <div style="background:white;border-radius:16px;padding:36px;border:1px solid rgba(180,140,80,0.15);margin-bottom:28px;">
      <p style="font-size:17px;color:#5c4a35;line-height:1.9;font-style:italic;margin:0 0 20px;">
        Dear ${firstName},
      </p>
    ...
    <p style="font-size:12px;color:#c4a882;text-align:center;margin-top:24px;font-family:'Arial',sans-serif;">
      MyStory.Family · <a href="https://mystory.family" style="color:#c4a882;">mystory.family</a>
    </p>
...
export default async function handler(req, res) {
  ...
  // It finds users who signed up 3 or 7 days ago and haven't completed their book
  const now = new Date();
  ...
  const day3Users = await supabase(
    `mystory_users?created_at=gte.${threeDaysAgo}T00:00:00&created_at=lt.${threeDaysAgo}T23:59:59&select=first_name,email`,
    "GET"
  );

  // Get users who signed up 7 days ago
  const day7Users = await supabase(
    `mystory_users?created_at=gte.${sevenDaysAgo}T00:00:00&created_at=lt.${sevenDaysAgo}T23:59:59&select=first_name,email`,
    "GET"
  );
  ...
  for (const user of (day3Users.data || [])) {
    try {
      await sendReminder(user.first_name, user.email, 3);
      results.day3++;
  ...
  for (const user of (day7Users.data || [])) {
    try {
      await sendReminder(user.first_name, user.email, 7);
      results.day7++;
    } catch (e) { results.errors.push(user.email); }
  }
