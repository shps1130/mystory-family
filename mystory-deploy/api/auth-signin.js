      ...
      if (paymentSuccess) {
        if (raw) {
          try {
            ...
            if (s.user?.email) {
              localStorage.setItem("mystory_paid_" + s.user.email, "true");
              // Mark as paid in Supabase — simple and reliable
        ...
        // Fallback — use pending email
        const emailToUse = pendingEmail;
        if (emailToUse) {
          localStorage.setItem("mystory_paid_" + emailToUse, "true");
          fetch("/api/paid-status", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
